...
public class ChessManagerTests {
    ...
    private (int id, MockPlayer white, MockPlayer black) SetupGame(ChessManager manager, string? fen = null) {
    ...
    public async Task CreateGame_And_RegisterPlayers_StartsAndFinishesGame(int maxConcurrent) {
        ...
        var (id, white, black) = SetupGame(manager, MateInOneFen);
    ...
    public async Task MaxConcurrentGames_IsRespected() {
        ...
        var (id2, white2, _) = SetupGame(manager);
    ...
    [Fact]
    public void CleanupLoop_RemovesExpiredBuilders() {
        // Arrange
